...
# For testing purposes setup variable can be set only once for every GUPB version
#SETUP=
SETUP="anything could be written here"
...
python3 -m venv $venv_name >> $setup_logs_file || { echo "Unable to create virtual environment"; exit 1; }

source $venv_name/bin/activate || { echo "Unable to enter virtual environment"; exit 1; }
